...
bool Mesh::load() {
	...
	while (getline(file, line)) {
		...
		if (token == "v") {
			...
			if (stream.fail()) {
				std::cerr << "normal syntax error on line " << line_num
						<< std::endl;
				return false;
			}
			...
			if (stream.fail()) {
				std::cerr << "uv syntax error on line " << line_num
						<< std::endl;
				return false;
			}
			...
			if (num_vertex > 4 || num_vertex < 3) {
				std::cerr << "Syntax error at line " << line_num
						<< ", face has incorrect number of vertices"
						<< std::endl;
				return false;
			}
			...
			for (size_t i = 0; i < num_vertex; ++i) {
				switch (format) {
				case VERTEX_ONLY:
					sscanf(face_tokens[i].c_str(), scan_vertex, &tri[i].vertex);
					tri[i].normal = 0;
					tri[i].tcoord = 0;
					break;

				case VERTEX_UV:
					sscanf(face_tokens[i].c_str(), scan_vertex_uv,
							&tri[i].vertex, &tri[i].tcoord);
					tri[i].normal = 0;
					break;

				case VERTEX_NORMAL:
					sscanf(face_tokens[i].c_str(), scan_vertex_normal,
							&tri[i].vertex, &tri[i].normal);
					tri[i].tcoord = 0;
					break;

				case VERTEX_UV_NORMAL:
					sscanf(face_tokens[i].c_str(), scan_vertex_uv_normal,
							&tri[i].vertex, &tri[i].tcoord, &tri[i].normal);
					break;

				default:
					std::cerr
							<< "Syntax error, unrecongnized face format at line "
							<< line_num << std::endl;
					break;
				}
			}
	...
	for (size_t i = 0; i < face_list.size(); ++i) {
		...
		for (size_t j = 0; j < 3; ++j) {
			...
			if (vidx < 0 || vidx >= num_vertex || nidx < -1
					|| nidx >= num_normal || tidx < -1 || tidx >= num_tcoord) {
	...
	for (size_t i = 0; i < face_list.size(); ++i) {
		...
		for (size_t j = 0; j < 3; ++j) {
			...
			std::pair<VertexMap::iterator, bool> rv = vertex_map.insert(
					std::make_pair(face.v[j], vert_idx_counter));
			if (rv.second) {
				MeshVertex v;
...
void Mesh::render() const {
	...
	glInterleavedArrays( GL_T2F_N3F_V3F, VERTEX_SIZE * sizeof vertex_data[0],
			&vertex_data[0]);
	glDrawElements( GL_TRIANGLES, index_data.size(), GL_UNSIGNED_INT,
			&index_data[0]);
}
...
bool Mesh::hit(const Ray ray, const real_t start, const real_t end,
	...
	real_t a = vertices[v0].position.x - vertices[v1].position.x;
	real_t b = vertices[v0].position.y - vertices[v1].position.y;
	real_t c = vertices[v0].position.z - vertices[v1].position.z;
	real_t d = vertices[v0].position.x - vertices[v2].position.x;
	real_t e = vertices[v0].position.y - vertices[v2].position.y;
	real_t f = vertices[v0].position.z - vertices[v2].position.z;
	real_t g = ray.d.x;
	real_t h = ray.d.y;
	real_t i = ray.d.z;
	real_t j = vertices[v0].position.x - ray.e.x;
	real_t k = vertices[v0].position.y - ray.e.y;
	real_t l = vertices[v0].position.z - ray.e.z;
